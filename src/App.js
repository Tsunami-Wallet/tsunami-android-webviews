...
const from = '0xe45D59f09A3c84bD083EE8a3148075a392187ceF'
...
const App = () => {
  ...
  async function deposit() {
    ...
    const tx = await contract.methods.deposit(toHex(deposit.commitment)).send({ value: toWei(AMOUNT), from: from, gas:2e6 })
    console.log(`https://kovan.etherscan.io/tx/${tx.transactionHash}`)
    console.log(`tornado-eth-${AMOUNT}-${netId}-${toHex(deposit.preimage, 62)}`)
    return `tornado-eth-${AMOUNT}-${netId}-${toHex(deposit.preimage, 62)}`
  }


  async function redeemNote () {





    const noteToRedeem = 'tornado-eth-1-42-0x60cb544a94946b72bd54234708b8898e497dbc987c78d76d7908ee2a2428a3ef70e72badd58ae54601de557e3dad37beee804f9f2ca34d30faf254893a62'
    contract = new web3.eth.Contract(require('./build/contracts/ETHTornado.json').abi, CONTRACT_ADDRESS)
    const deposit = parseNote(noteToRedeem)
    const { proof, args } = await generateSnarkProof(deposit, from)
  ...
  // Notes for moe:
  // 1. Use react app to create the deposit.
  // 2. create a simpler web app based off of websnark example to generate the withdrawal.
